...
class NotificationsScreen extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: context.select((NotificationsBloc bloc) => Text("Status: ${bloc.state.status.name}")),
        actions: [
          IconButton(onPressed: () => context.read<NotificationsBloc>().requestPermission(), icon: const Icon(Icons.settings))
        ],
      ),
      body: _HomeView(),
    );
...
class _HomeView extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    if(notifications.isEmpty){
      return Text('No hay notificaciones');
    }

    return ListView.builder(itemCount: notifications.length, itemBuilder: (buildContext, index) {
...
class _NotificationItem extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: pushMessage.imageUrl != null ? Image.network(pushMessage.imageUrl!, width: 100, height: 60,) : null,
